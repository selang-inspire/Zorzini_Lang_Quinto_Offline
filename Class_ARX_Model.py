...
class ARDL(AutoReg):
    ...
    def __init__(
        ...
        *,
        ...
        #Set wights to zero were indices matches
        self.indicesForWeightZero = None
    ...
    def _fit(
        ...
        #ols_mod = OLS(self._y, self._x) #originally OLS

        ###############################################################
        #New Part to set weights to zero were indices matches
        ###############################################################
        #Set wights to zero were indices matches
        positions = self.indicesForWeightZero
        weights = np.ones(self._y.shape[0])
        shifts = self.data.orig_exog.shape[0] - self._x.shape[0]
        shifts_endog = self.data.orig_endog.shape[0] - self._y.shape[0]
        if shifts_endog > shifts:
            shifts = shifts_endog
        ...
        ols_mod = WLS(self._y, self._x, weights = weights) #originally OLS, but weights adjsutment not possible
        ###############################################################
